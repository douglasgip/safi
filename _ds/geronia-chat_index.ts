...
function pedidosSummaryText(labels: string[], rows: any[]) {
  ...
  const blocks = labels.map(label => {
    ...
    const linhas = list.map(r => {
      ...
      let qtdTxt = ''
      if (r.qtd_produtos_previsto != null || r.num_produtos_nf != null) {
        const prev = r.qtd_produtos_previsto
        const real = r.num_produtos_nf
        if (prev != null && real != null) {
          qtdTxt = prev === real ? `, ${real} produto(s) na NF (bate com o previsto)` : `, ${real} produto(s) na NF — DIVERGENTE da quantidade prevista de ${prev}`
        } else if (real != null) {
          qtdTxt = `, ${real} produto(s) na NF`
        } else {
          qtdTxt = `, quantidade prevista de ${prev} produto(s) (ainda não conferida)`
        }
      }
